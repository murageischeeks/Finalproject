@php
    if (Auth::check()) {
        ...
        if ($user->role === 'doctor') {
            ...
            $profileRoute = '#'; // Admin profile route (if exists)
        } else {
            $dashboardRoute = route('patient.dashboard');
            $profileRoute = route('patient.profile');
        }
    } else {
        // Default route for guests (before login/registration)
        $dashboardRoute = url('/');
        $profileRoute = '#';
    }
@endphp
                ...
                @auth
                    <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                        <x-nav-link :href="$dashboardRoute"
                            :active="request()->is('doctor/dashboard') || request()->is('patient/dashboard') || request()->is('admin/dashboard')">
                            {{ __('Dashboard') }}
                        </x-nav-link>
                    </div>
                @endauth
